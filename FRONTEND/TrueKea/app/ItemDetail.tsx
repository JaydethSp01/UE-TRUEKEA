...
export default function ItemDetailScreen() {
  ...
  const loadMyItems = async () => {
    try {
      ...
      setMyItems(res.data.filter((item) => item.id !== Number(id)));
    } catch (error) {
    ...
    if (myItems.length === 0) {
      Alert.alert(
        ...
        [
          ...
          { text: "Publicar ahora", onPress: () => router.push("/add-item") },
        ]
      );
      return;
    }
  ...
  const handleConfirmSwap = async () => {
    if (!selectedItemForSwap || !item) return;
    ...
    try {
      ...
      Alert.alert(
        ...
        [{ text: "OK", onPress: () => router.push("/(tabs)/swaps") }]
      );
  ...
  const handleChat = () => {
    router.push(`/chat/${item?.owner.id}`);
  };
  ...
  const isMyItem = item.owner.id === user?.id;
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.imageContainer}>
          <Image source={{ uri: item.img_item }} style={styles.image} />
        ...
        <View style={styles.content}>
          ...
          <View style={styles.co2Container}>
            ...
            <Text style={styles.co2Text}>Ahorro de CO₂: {item.value} kg</Text>
          </View>
